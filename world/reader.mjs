...
export function createReader(){
 ...
 async function open(record){
  ...
  try{
   ...
   status.textContent=mark?'記録した位置から。本文は公開原本のままです。':'本文は公開原本のままです。「読書位置を記録」を押すと、このブラウザーに位置を保存します。';dialog.dispatchEvent(new CustomEvent('akashic:reader-open',{detail:{record:record.id}}));return true;
  }catch(error){if(own===request&&dialog.open)status.textContent='本文を読み込めませんでした。記録画面の「原本をひらく」から読むか、もう一度試してください。';return false;}
  finally{clearTimeout(timer);}
 }
 $('#reader-close').addEventListener('click',()=>{remember();dialog.close();});dialog.addEventListener('cancel',remember);dialog.addEventListener('close',()=>{if(dialog.open)return;remember();++request;controller?.abort();cancelAnimationFrame(paint);paint=0;});
 ...
 function capturePosition(){return dialog.open&&current&&content.children.length?readingMark({schema:1,id:current.id,ratio:ratio(),font}):null;}
 function restorePosition(value){let mark;try{mark=readingMark(value);}catch{return false;}if(!dialog.open||current?.id!==mark.id)return false;const own=request;memory.set(mark.id,mark);fontSize(mark.font);requestAnimationFrame(()=>{if(own!==request||!dialog.open)return;scroll.scrollTo({top:mark.ratio*Math.max(0,scroll.scrollHeight-scroll.clientHeight),behavior:'instant'});progress();});return true;}
 return {open,capturePosition,restorePosition,close(){if(dialog.open){remember();dialog.close();}},getState:()=>({record:current?.id||null,open:dialog.open,font,ratio:ratio(),blocks:content.children.length}),dispose(){remember();++request;controller?.abort();cancelAnimationFrame(paint);if(dialog.open)dialog.close();cache.clear();}};
}
