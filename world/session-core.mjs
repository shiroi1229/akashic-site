import {readingMark} from './reader.mjs';
...
export function terminalSession(value){
 ...
 const reading=value.reading==null?null:readingMark(value.reading);if(reading&&Number(reading.id.match(/^ep01-ch([1-6])/)[1])>s.chapter)throw Error('session_reading_chapter');
 return {...(reading?{reading}:{}),schema:SESSION_SCHEMA,state:{query:s.query,kind:s.kind,chapter:s.chapter,savedOnly:s.savedOnly},graph:{view:graphView(g.view),list:g.list},scrollY:y,selected};
}
