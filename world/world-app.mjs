...
async function boot(){
 try{const embedded=document.querySelector('#world-manifest');let w;if(embedded){w=JSON.parse(embedded.textContent);}else{const r=await fetch(new URL('../experience-base/world/manifest.json',import.meta.url),{credentials:'same-origin'});if(!r.ok)throw Error('manifest_missing');w=await r.json();}window.worldApp=createWorldApp(w); const visitor=await import('./world-visitor.mjs'); window.worldVisitor=visitor.enhanceWorld(w,window.worldApp); const enhanced=await import('./experience.mjs');window.worldExperience=enhanced.installExperience(w,window.worldApp);document.dispatchEvent(new Event('akashic:ready'));}
